...
window.NoteScene = (() => {
  ...
  let rimMesh = null;
  function buildRimShell() {
    const geo = new THREE.SphereGeometry(1, 48, 36);
    const pos = geo.attributes.position, v = new THREE.Vector3();
    for (let i = 0; i < pos.count; i++) {
      v.fromBufferAttribute(pos, i);
      const folds = fbm(v.x*2.2+3, v.y*2.2+7, v.z*2.2+1) * 0.07;
      v.normalize().multiplyScalar(1 + folds);
      // slight hemisphere cleft
      v.y *= 1 - Math.exp(-Math.abs(v.x)*8) * 0.05;
      pos.setXYZ(i, v.x * 2.1, v.y * 1.56, v.z * 2.52);
    }
    geo.computeVertexNormals();
    const mat = new THREE.ShaderMaterial({
      transparent: true, blending: THREE.AdditiveBlending, depthWrite: false,
      uniforms: { uIntensity: { value: 0.38 } },
      vertexShader: `varying vec3 vN; varying vec3 vP;
        void main() { vN = normalize(normalMatrix * normal); vP = position;
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
      fragmentShader: `varying vec3 vN; varying vec3 vP; uniform float uIntensity;
        void main() {
          float rim = pow(1.0 - abs(vN.z), 3.2);
          vec3 violet = vec3(0.545, 0.486, 1.0);
          vec3 cyan = vec3(0.176, 0.886, 0.902);
          vec3 col = mix(cyan, violet, smoothstep(-0.7, 0.7, vP.x));
  ...
  // 3) EXPLORE LOD — one denser dendrite layer, built lazily past a zoom threshold
  const lodZ = () => Math.min(9, fitZ() * 0.55);
  let detailGroup = null, detailMats = [];
  function buildDetailLayer() {
    detailGroup = new THREE.Group();
    detailMats = [];
    const d = { nodes: Math.round(DENSITY.nodes * 0.8), branches: DENSITY.branches + 2 };
    for (const reg of REGIONS) {
      const g = buildRegion(reg, d);
      g.userData.branchMat.opacity = 0;
      g.userData.somaMat.opacity = 0;
      detailMats.push(g.userData.branchMat, g.userData.somaMat);
      detailGroup.add(g);
    }
    neuralGroup.add(detailGroup);
  }
  ...
  function frame() {
    ...
    if (rimMesh) {
      const tgt = exploreMode ? 0.55 : 0.36;
      const u = rimMesh.material.uniforms.uIntensity;
      u.value += (tgt - u.value) * 0.08;
    }
    // LOD: densify when zoomed close in explore mode
    if (exploreMode && camZ < lodZ() && !detailGroup) buildDetailLayer();
    if (detailGroup) {
      const on = exploreMode && camZ < lodZ();
      for (const m of detailMats) {
        const tgt = on ? (m.size ? 0.7 : 0.3) : 0;   // points vs lines
        m.opacity += (tgt - m.opacity) * 0.08;
      }
      detailGroup.visible = detailMats.some(m => m.opacity > 0.02);
    }
